...
ssh $SSH_OPTS $EC2_USER@$EC2_IP << EOF
  ...
  # Stop and remove any existing container using the target port
  EXISTING_CONTAINER=\$(sudo docker ps --filter "publish=$HOST_PORT" --format "{{.Names}}" | head -1)
  if [ ! -z "\$EXISTING_CONTAINER" ]; then
    echo "Stopping and removing existing container using port $HOST_PORT: \$EXISTING_CONTAINER"
    sudo docker stop \$EXISTING_CONTAINER || true
    sudo docker rm \$EXISTING_CONTAINER || true
  fi
  
  # Also stop and remove the container with our specific name if it exists
